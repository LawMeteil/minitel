...
def interfaces():
    res = subprocess.check_output(['/home/minitel/functions.sh', 'network']).decode("ascii");
    list = [];
...
def routes():
    res = subprocess.check_output(['/home/minitel/functions.sh', 'route']).decode("ascii")
    info = json.loads(res);
...
def forward():
    res = subprocess.check_output(['/home/minitel/functions.sh', 'forwrd']).decode("ascii")
    if (res != 0):
        res = "FALSE"
    else:
        res = "TRUE"
