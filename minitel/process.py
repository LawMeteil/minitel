...
def show_process(stdscr,sort = "pid"):
    ...
    if key == 49:
        ...
        while key == 55:
            stdscr.clear()
            if invert == 0:
                print_center(stdscr, "1:pid  2:comm  3:stat  4:%cpu  5:%mem  6:ppid  7:invert")
            else:
                print_center(stdscr, "1:pid  2:comm  3:stat  4:%cpu  5:%mem  6:ppid  7:normalize")
            key = stdscr.getch()
            if key == 55:
                if invert == 0:
                    invert = 1
...
# boucle qui s'actualise toute les secondes ???
def process(sort):
    str = subprocess.check_output(['/home/minitel/functions.sh', 'proc', sort]).decode("ascii")
    Process = []
